...
Select a folder and set the Threshold slider to how strict or lax do you want the similarity algorithm to be (lower means tighter threshold with 0 only selecting the exact same images). Check the file list manually and select all the duplicate files and send them to the Recycle bin.

Between two similar files, the one with larger dimensions and file size is chosen as original. Double clicking images will open them.
